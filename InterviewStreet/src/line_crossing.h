...
// https://zulily.interviewstreet.com/challenges/dashboard/#problem/4fdc7ebdd8d96
...
vec start_x(N_MAX), start_y(N_MAX), end_x(N_MAX);
int test[N_MAX] = {0};

int main()
{
	int N = input.readInt(), Q, x, y, l, i;
	
	for(i = 0; i < N; i++)
	{
		start_x[i].first = input.readInt();
		start_x[i].second = i;

		start_y[i].first = input.readInt();
		start_y[i].second = i;

		end_x[i].first = start_x[i].first + input.readInt();
		end_x[i].second = i;
	}

	sort(start_x.begin(), start_x.begin() + N, comp);
	sort(start_y.begin(), start_y.begin() + N, comp);
	sort(end_x.begin(), end_x.begin() + N, comp);
	...
	for(i = 0; i < Q; i++)
	{
		...
		// [y1, y2)
		vec::iterator y1 = lower_bound(start_y.begin(), start_y.begin() + N, Data(y, -1), comp);
		vec::iterator y2 = upper_bound(y1, start_y.begin() + N, Data(y + l, -1), comp);

		vec::iterator x1 = upper_bound(start_x.begin(), start_x.begin() + N, Data(x, -1), comp);
		vec::iterator x2 = lower_bound(end_x.begin(), end_x.begin() + N, Data(x, -1), comp);
		...
		for(it = start_x.begin(); it != x1; ++it)
		{
			test[it->second]++;
		}

		int count(0);
		for(it = x2, end = end_x.begin() + N; it != end; ++it)
		{
			if(test[it->second] == 2)
				count++;
		}
		...
		output(count, "\n");
		memset(test, 0, N_MAX * sizeof(int));
	}
