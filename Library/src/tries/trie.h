/**
 * Shamelessly copied from https://code.google.com/p/simple-trie/source/browse/inc/trie.h
 */
#ifndef TRIE_H
#define TRIE_H
...
template<typename T, typename V, typename Cmp, typename Items> class Node;

template<typename T, typename V, typename Cmp, typename Items> class NodeItem {
private:
...
public:
	NodeItem(const T &endSymbol, T const &key) :
			mEndSymbol(endSymbol), mKey(key), mChilds(0) {
	}
...
template<typename T, typename V, typename Cmp, typename Items> class EndNodeItem: public NodeItem<T,
		V, Cmp, Items> {
private:
...
public:
	...
	EndNodeItem(const T &endSymbol, T const &key, V const &value) :
			ParentClass(endSymbol, key), mValue(value) {
	}
...
template<typename T, typename V, typename Cmp, typename Items> class NodeItemPtrCompare {
private:
	typedef NodeItem<T, V, Cmp, Items> NodeItemClass;
...
public:
	...
	public:
		ConstIterator(const NodeClass *node, const NodeClass * root, const T * key = 0,
				bool mooveToEnd = false) :
				mRootNode(root), mCurrentNode(node), mCheckKeyLeft(false), mCheckKeyRight(true), mEndReached(
						false) {
			if (!root) {
				mRootNode = node;
			}
		...
		ConstIterator(const ConstIterator & oth) :
				mRootNode(oth.mRootNode), mCurrentNode(oth.mCurrentNode), mCurrentPos(
						oth.mCurrentPos), mKeyStack(oth.mKeyStack), mCheckKeyLeft(
						oth.mCheckKeyLeft), mCheckKeyRight(oth.mCheckKeyRight), mEndReached(
						oth.mEndReached) {
			if (!mKeyStack.empty()) {
	...
	protected:
			...
			while (mEndReached || !isLeftEnd()) {
				...
				if (!newNode && !mKeyStack.empty()) {
					if (mCheckKeyLeft) {
						...
						ItemsContainerConstIter iter = mCurrentNode->mItems.find(
								mCurrentNode->endSymbol());
		...
		bool equals(const ConstIterator & oth) const {
			...
			if (this->mCurrentNode == oth.mCurrentNode && this->mCurrentPos == oth.mCurrentPos) {
				return true;
			}
...
public:
	Node(const T &eSymbol, NodeClass * parent = 0) :
			mItems(eSymbol), mEndSymbol(eSymbol), mSize(0), mParent(parent) {
	}
 ...
 */
template<typename T, typename V, typename Cmp, int Max = 256, typename M = SymbolToIndexMapper<T> > class VectorItems {
public:
...
public:
	VectorItems(T const &endSymbol) :
			mEndSymbol(endSymbol), mItems(Max, (Item *) 0) {
	}
